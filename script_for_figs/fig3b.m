...
L = 100;
% D=1
disp('loading data files for plotting FIG. 3b...')
load(['./input/movies/moving_clusters.mat'])
load(['./input/movies/moving_clusters_gr.mat'])
dyn = load(['./input/movies/moving_clusters_dyn.dat']);
disp('data loaded!')
%%
% create a new figure window
set(gcf,'visible','off');
clf;
disp('Generating the figure: spatial profiles and pair correlaion of snapshots for Dn=1')
gcf_print_format(gcf, 'w', [15 8]);

% assign time frames for plotting
t_s = [8, 90, 180];     % frame of time to plot
for ii = 1:length(t_s)
...
spatial_figure(M_plot{1}, dyn, 1, 100, 0)
...
spatial_figure(M_plot{2}, dyn, 1, 100, 0)
line1 = '$D_n=1$';
...
spatial_figure(M_plot{3}, dyn, 1, 100, 0)
...
print(gcf,['./output/fig3b'],'-dpdf','-r600')
disp('figure generated!')
disp('FIG. 3b was generated!')